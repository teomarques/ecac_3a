...
O trabalho está estruturado de acordo com as tarefas propostas no guião do Trabalho Prático 1 (TP1.pdf). Cada secção deste relatório corresponde a uma tarefa, explicando a metodologia adotada, as funções implementadas no script \texttt{mainActivity\_py}, e os resultados obtidos, incluindo gráficos e tabelas.

\section{Tarefa 2: Carregamento de Dados}
A primeira etapa consistiu em desenvolver uma função para carregar os dados de um participante a partir dos ficheiros CSV. A função \texttt{carregar\_dados\_participante} lê os 5 ficheiros de um determinado participante e concatena-os num único array NumPy para facilitar a manipulação posterior.
